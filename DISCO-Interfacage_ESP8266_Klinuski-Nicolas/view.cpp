    ...
    if (isInit == 1) {
        ...
        { 
            if (valReaded > 0)
            {
                    ...
                    if (valReaded < 9)                                                                   //Si la valeur est inférieur à 9 alors supprimer les 6 premiers caractères (caractère invisible de base)
                    {
                        memcpy(donneecpy, &(donnee[6]), (valReaded > 100) ? 100 : valReaded);            //Si la chaine de caractère est supérieur à 100, alors mettre seulement les 100 premiers
                    }
                    if (valReaded >= 9 && valReaded <= 15)                                               //Si la valeur est supérieur ou égal à 9 et est inférieur ou égal à 15 alors supprimer les 7 premiers caractères
                    {
                        memcpy(donneecpy, &(donnee[7]), (valReaded > 100) ? 100 : valReaded);
                    }
                    if (valReaded > 15)                                                                  //Si la valeur est supérieur ou égal à 9 alors supprimer les 8 premiers caractères
                    {
